...
def main():
    ...
    # Emission checker
    # Setup the model with your existing frequency dicts
    model = EmissionModel(yri_freqs, ceu_freqs)

    # Let's test the High-Signal SNP you found
    pos = 16201114 
    # Imagine our admixed person has the '1|1' genotype here
    genotype = (1, 1)

    probs = model.get_emission_probs(pos, genotype)

    print(f"Genotype {genotype} at {pos}:")
    print(f"Likelihood if YRI: {probs['YRI']:.4f}")
    print(f"Likelihood if CEU: {probs['CEU']:.4f}")
